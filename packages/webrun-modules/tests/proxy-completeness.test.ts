...
const depSource: Source = {
  ...
  async load() {
    const files = new MemFilesApi();
    await writeText(files, "/index.js", `export const alpha = 1;\nexport const beta = 2;`);
    return {
      name: "dep",
      version: "1.0.0",
      files,
...
async function proxyAfter(secondImport: string, files: string[]): Promise<string> {
  const server = newModuleServer({
    cache: new MemFilesApi(),
    sources: [appSource(secondImport), depSource],
  });
